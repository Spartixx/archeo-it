...
<div class="flex flex-col gap-[7rem] justify-between p-5">
    ...
    <div class="px-6">
        <div class="w-full flex flex-col justify-start items-center gap-10 rounded-xl border">
            <div class="w-full flex flex-col gap-5 justify-start items-center">
                <!-- Boutons -->
                <div class="w-full h-20 flex flex-row justify-between ">
                    <button onclick="showSection('blog')" id="btn-blog"
                            class="flex flex-row items-center justify-center sm:text-sm md:text-lg p-2 w-full bg-yellow-950/40 text-black rounded-lg transition hover:bg-yellow-950 hover:text-white">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-2 h-5 w-5">
                    ...
                    <button onclick="showSection('chantiers')" id="btn-chantiers"
                            class="flex flex-row items-center justify-center sm:text-sm md:text-lg p-2 w-full bg-yellow-950/40 text-black rounded-lg transition hover:bg-yellow-950 hover:text-white">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-2 h-5 w-5">
            ...
            <!-- Section Blog -->
            <div id="section-blog" class="flex flex-row justify-around flex-wrap w-full gap-5 p-3">
                ...
                <!-- Duplique cette carte pour plus d'articles -->
            </div>

            <!-- Section Chantiers -->
            <div id="section-chantiers" class="flex flex-row justify-around flex-wrap w-full gap-5 p-3 hidden">
                <div class="max-w-sm rounded-xl overflow-hidden shadow-lg bg-white">
                    <img class="w-full" src="../assets/img/logo-archeo-it.png" alt="Chantier 1">
                    <div class="px-6 py-4">
                        <div class="font-bold text-xl mb-2">Chantier à Gizeh</div>
                        <p class="text-gray-700 text-base">Exploration autour des pyramides d'Égypte.</p>
                    </div>
                </div>
                <div class="max-w-sm rounded-xl overflow-hidden shadow-lg bg-white">
                    <img class="w-full" src="../assets/img/logo-archeo-it.png" alt="Chantier 2">
                    <div class="px-6 py-4">
                        <div class="font-bold text-xl mb-2">Ruines Mayas</div>
                        <p class="text-gray-700 text-base">Travaux de fouilles dans la jungle guatémaltèque.</p>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function showSection(section) {
        const blog = document.getElementById('section-blog');
        const chantiers = document.getElementById('section-chantiers');
        const btnBlog = document.getElementById('btn-blog');
        const btnChantiers = document.getElementById('btn-chantiers');

        if (section === 'blog') {
            blog.classList.remove('hidden');
            chantiers.classList.add('hidden');
            btnBlog.classList.add('bg-yellow-950', 'text-white');
            btnBlog.classList.remove('bg-yellow-950/40', 'text-black');
            btnChantiers.classList.remove('bg-yellow-950', 'text-white');
            btnChantiers.classList.add('bg-yellow-950/40', 'text-black');
        } else {
            chantiers.classList.remove('hidden');
            blog.classList.add('hidden');
            btnChantiers.classList.add('bg-yellow-950', 'text-white');
            btnChantiers.classList.remove('bg-yellow-950/40', 'text-black');
            btnBlog.classList.remove('bg-yellow-950', 'text-white');
            btnBlog.classList.add('bg-yellow-950/40', 'text-black');
        }
    }
</script>
